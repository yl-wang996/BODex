...
class PrimitiveCollisionCostConfig(CostConfig):
    ...
    # only work for the contact object
    ...
    # only used for contact strategy
    total_spheres: int = 0
...
class PrimitiveCollisionCost(CostBase, PrimitiveCollisionCostConfig):
    def __init__(self, config: PrimitiveCollisionCostConfig):
        ...
        if self.contact_strategy is not None:
            new_dict = {
                'opt_progress': self.tensor_args.to_device(self.contact_strategy['opt_progress']),
                'distance': self.tensor_args.to_device(self.contact_strategy['distance']).view(-1, 1).repeat(1, self.total_spheres)
            }
            non_contact_idx = [i for i in range(self.total_spheres) if i not in self.contact_points_idx]
            new_dict['distance'][:, non_contact_idx] = torch.clamp(new_dict['distance'][:, non_contact_idx], min=0.002)
            new_dict['distance'][:, self.contact_points_idx] -= 0.002
            self.contact_strategy = new_dict
        else:
            self.contact_strategy = {}
            self.contact_strategy['opt_progress'] = self.tensor_args.to_device([0.0])
            self.contact_strategy['distance'] = self.tensor_args.to_device([0.0]*self.total_spheres).unsqueeze(0)
            self.contact_strategy['distance'][:] = self.activation_distance
        
    def _get_contact_stage(self, opt_progress):
        if len(self.contact_strategy['opt_progress']) == 1:
            contact_distance = self.contact_strategy['distance'][0]
        else:
            contact_stage = (self.contact_strategy['opt_progress'] <= opt_progress).int().sum() - 1
            contact_distance = self.contact_strategy['distance'][contact_stage]
            self.activation_distance[:] = max(contact_distance.max(), 0.005)
        return contact_distance
    
    def update_contact_distance(self, new_contact_distance: float):
        assert len(self.contact_strategy['opt_progress']) == 1
        self.contact_strategy['distance'][:] = new_contact_distance
        return 
    
    def sweep_kernel_fn(self, robot_spheres_in, env_query_idx: Optional[torch.Tensor] = None, opt_progress = 0.0):
        ...
        dist = self.sweep_check_fn(
            ...
            contact_distance=self._get_contact_stage(opt_progress),
    ...
    def sweep_fn(self, robot_spheres_in, env_query_idx: Optional[torch.Tensor] = None, opt_progress=1.0):
    ...
    def discrete_fn(self, robot_spheres_in, env_query_idx: Optional[torch.Tensor] = None, opt_progress=1.0):
