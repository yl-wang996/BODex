...
std::vector<torch::Tensor> self_collision_distance_wrapper(
    ...
    const int ndpt = 8, const bool debug = false, const float activation_distance = 0.0) {
  ...
  return self_collision_distance(
      ...
      thread_locations_size, batch_size, nspheres, compute_grad, ndpt, debug, activation_distance);
}
