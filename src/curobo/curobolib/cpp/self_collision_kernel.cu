...
#pragma unroll 4
    ...
    __global__ void self_collision_distance_kernel7(
      ...
      const float activation_distance = 0.0)
    {
...
#pragma unroll
        ...
        for (uint16_t l = 0; l < nbpb; l++) // iterate through nbpb batches
        {
          ...
          float d = sqrt((sph1.x - sph2.x) * (sph1.x - sph2.x) +
                         (sph1.y - sph2.y) * (sph1.y - sph2.y) +
                         (sph1.z - sph2.z) * (sph1.z - sph2.z));
          float f_diff = r_diff - d + activation_distance;
...
#pragma unroll
        ...
        for (uint8_t l = 0; l < nbpb; l++)
        {
          ...
          //////////////////////////////////////////////////////
          if (max_d.d != 0.0) {
            if (max_d.d < activation_distance && activation_distance != 0.0) {
              out_distance[batch_idx + l] = weight[0] * 0.5 / activation_distance * max_d.d * max_d.d;
            }
            else {
              out_distance[batch_idx + l] = weight[0] * max_d.d - 0.5 * activation_distance;
            }

            if (write_grad) {
              float4 sph1_4 = *(float4 *)&robot_spheres[4 *
                                            ((batch_idx + l) * nspheres + max_d.i)];
              float4 sph2_4 = *(float4 *)&robot_spheres[4 *
                                            ((batch_idx + l) * nspheres + max_d.j)];
              float3 sph1 = *(float3 *)&sph1_4;
              float3 sph2 = *(float3 *)&sph2_4;
              float3 dist_vec = normalize(sph1 - sph2);
              if (max_d.d < activation_distance && activation_distance != 0.0) {
                dist_vec = dist_vec * max_d.d / activation_distance; 
              }
...
std::vector<torch::Tensor> self_collision_distance(
    ...
    const bool experimental_kernel = false, const float activation_distance = 0.0) {
        ...
        robot_spheres.scalar_type(), "self_collision_distance", ([&] {
          self_collision_distance_kernel7<scalar_t, 1, nbpb>
              <<<blocksPerGrid, threadsPerBlock, smemSize, stream>>>(
                  ...
                  compute_grad, activation_distance);
        }));
    }
