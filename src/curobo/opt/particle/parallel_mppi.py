...
class ParallelMPPIConfig(ParticleOptConfig):
    ...
    def __post_init__(self):
        if self.use_root_pose and isinstance(self.init_cov, List) and len(self.init_cov) == 2:
            self.init_cov = self.tensor_args.to_device(
                    [self.init_cov[0]] * 7 + [self.init_cov[1]] * (self.d_action-7)
                )
        else:
...
class ParallelMPPI(ParticleOptBase, ParallelMPPIConfig):
    ...
    def sample_actions(self, init_act):
        ...
        act_seq = scale_ctrl(act_seq, self.tendon_joints, self.use_root_pose, self.action_lows, self.action_highs, squash_fn=self.squash_fn)
