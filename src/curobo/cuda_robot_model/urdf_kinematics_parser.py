...
class UrdfKinematicsParser(KinematicsParser):
    ...
    def __init__(
        ...
        """
        if coacd_obj_version:
            # only for load decomposed object
            self._robot = yourdfpy.URDF.load(urdf_path, load_meshes=False)
        else:
    ...
    def get_link_mesh(self, link_name: str) -> CuroboMesh:
        ...
        mesh_list = []
        for i, visual_mesh in enumerate(link_data.visuals):
            m = visual_mesh.geometry.mesh
            mesh_pose = visual_mesh.origin
            ...
            mesh_list.append(CuroboMesh(
                name=link_name + '_' + str(i),
                pose=mesh_pose,
                scale=m.scale,
