import numpy as np
import hppfcl_openmp_wrapper
import time 
import hppfcl 

def loadConvexMesh(file_name: str, scale: np.ndarray):
    loader = hppfcl.MeshLoader()
    bvh: hppfcl.BVHModelBase = loader.load(file_name, scale)
    bvh.buildConvexHull(True, "Qt")
    return bvh.convex

def hppfcl_gjk_worker(shape1, rot1, trans1, shape2, rot2, trans2):
    ...
    hppfcl.distance(shape1, T1, shape2, T2, dist_req, dist_res)
...
for op in file_dict['obj']:
    ...
    om = hppfcl_openmp_wrapper.loadConvexMeshCpp(op, np.array([obj_scale]*3))
...
for rp in file_dict['robot']:
    ...
    rm = hppfcl_openmp_wrapper.loadConvexMeshCpp(rp, np.array([robot_scale]*3))
...
for i in range(100):
    ...
    hppfcl_openmp_wrapper.batched_hppfcl_distance(
    ...
    st2 = time.time()
    results = [hppfcl_gjk_worker(obj_mesh_list_debug[obj_idx_cpu[i]], obj_rot_cpu[i], obj_trans_cpu[i], robot_mesh_list_debug[robot_idx_cpu[i]], robot_rot_cpu[i], robot_trans_cpu[i]) for i in range(num_query)]
    st3 = time.time()
