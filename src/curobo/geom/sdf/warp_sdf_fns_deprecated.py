...
def get_swept_closest_pt_batch_env(
    ...
    mesh_allow_contact: wp.array(dtype=wp.uint64),
    ...
    while i < n_mesh:
        if mesh_enable[i] == uint_one:
            ...
            if contact_flag and mesh[i] == mesh_allow_contact[env_idx]:
                dist_bias = contact_distance[sph_idx] - eta
            else:
                dist_bias = 0.0
            ...
            if wp.mesh_query_point(
                ...
                dist = (-1.0 * dis_length * sign) + in_rad + dist_bias
                if dist > 0:
                    if dist == in_rad:
    ...
    if enable_speed_metric == 1 and (h_idx > 0 and h_idx < horizon - 1):
        ...
        sph_vel = wp.length(norm_vel_vec) + 1e-5
        if sph_vel > 1e-3:
    ...
    while i < n_mesh:
        if mesh_enable[i] == uint_one:
            ...
            if wp.mesh_query_point(
                ...
                delta = cl_pt - local_pt
                dis_length = wp.length(delta)
                dist = -1.0 * dis_length * sign + dist_bias
